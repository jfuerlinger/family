...
For Grok in this repo, install into `.grok/skills/` (this project gitignores `.agents/`):

```bash
npx skills add <owner/repo> --skill <name> -y --agent grok
```

`ctx7 skills install` flags for other agents:
```bash
...
All `ctx7 skills` commands accept these flags to target a specific AI coding assistant:
...
Grok in this repo has no `ctx7 --grok` flag. Install with `npx skills add <owner/repo> --skill <name> -y --agent grok` so files land in `.grok/skills/`.
