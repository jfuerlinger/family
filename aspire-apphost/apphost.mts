...
//
// NOTE: The `.aspire/` directory is auto-generated by the Aspire CLI on first run
// (`aspire run` / `npm run aspire:start`). It is not committed to source control.
// Run `npm run aspire:start` once and the tooling will generate it automatically.
...
await app.withEnvironment('AUTH_SECRET', process.env.AUTH_SECRET ?? (() => {
  if (process.env.NODE_ENV !== 'test') {
    console.warn('[AppHost] AUTH_SECRET is not set — using insecure dev default. Set AUTH_SECRET for production.');
  }
